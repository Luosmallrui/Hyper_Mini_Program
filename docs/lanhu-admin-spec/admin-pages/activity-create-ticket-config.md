...
## 页面用途
活动发布第 4 步，配置票券规格、价格、库存、限购、观演人数和启用状态。

## 页面结构
- 推荐路由：`pages/user-sub/organizer/index`
- 内部视图：`createWizard:step4`
- 当前状态：半完成
- 优先级：P0
- 预览资源：`assets/previews/admin-ticket-config.png`
...
- onAddTicketSpec/onDeleteTicketSpec/onSaveSpec：触发于 新增/删除/保存；成功反馈：更新规格列表；失败反馈：校验错误

## 表单说明
- ticketTypeName (string)：非空，最多 15 字
- ticketSpecs (array)：至少 1 项，最多 5 项
- price (number)：>=0
- stock (integer)：>=0
- limit (integer)：>=0
- attendees (integer)：>=1
...
## 与当前代码的差异
当前 ticketSpecs 已有基础结构；需补最多 5 项、名称 15 字、启用/禁用、日期、数字校验和保存行为。
