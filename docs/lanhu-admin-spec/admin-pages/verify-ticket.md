...
- onScanCode/onConfirmVerify：触发于 扫码/确认核销；成功反馈：成功弹窗 + 刷新已核销列表；失败反馈：失败弹窗

## 表单说明
- code (string)：券码不能为空；格式 unknown
- newVerifierName (string)：姓名不能为空
- newVerifierPhone (phone)：11 位手机号基础校验
...
- Proposed 接口：verifyTicketCode
- Mock：`src/pages/user-sub/organizer/mock.ts`

## 与当前代码的差异
当前 verify 页面是内部视图，未注册独立路由；扫码 mock 存在，但真实接口和券码校验缺失。
