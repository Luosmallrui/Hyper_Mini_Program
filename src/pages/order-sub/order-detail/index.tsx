...
export default function OrderDetailPage() {
  ...
  const getStatusConfig = () => {
    const configs = {
      ...
      used: {label: '已使用', color: '#999', icon: 'check-circle'},
  ...
  const handleRefund = () => {
    Taro.showModal({
      title: '申请退票',
      content: '确认要申请退票吗？退票后将按平台规则退还款项',
      confirmText: '确认退票',
      cancelText: '再想想',
      success: (res) => {
        if (res.confirm) {
    ...
    <View className='order-detail-page'>
      ...
      <View className='status-card' style={{backgroundColor: `${statusConfig.color}15`}}>
        ...
        <View className='status-info'>
          ...
          {orderDetail.status === 'paid' && (
            <Text className='status-tip'>请在活动开始前凭二维码入场</Text>
          )}
      ...
      <View className='section-card'>
        <View className='section-title'>
          <AtIcon value='file-generic' size='18' color='#333'/>
          <Text>活动信息</Text>
        </View>
      ...
      <View className='section-card'>
        <View className='section-title'>
          <AtIcon value='bookmark' size='18' color='#333'/>
          <Text>票务信息</Text>
        </View>
        ...
        <View className='info-row'>
          ...
          <Text className='info-value'>×{orderDetail.ticketCount}</Text>
        </View>
      ...
      {orderDetail.status === 'paid' && (
        <View className='bottom-actions'>
          <View className='action-btn secondary' onClick={handleContact}>
            <AtIcon value='phone' size='18' color='#666'/>
            <Text>联系客服</Text>
          </View>
